...
class PEAQ(object):
	...
	def process(self, referenceSignal, testSignal):
		...
		#Number of frames:
		self.Np = np.int(np.floor(len(sigR)/self.Nadv)-1)
		...
		for i in np.arange(self.Np):
		    xR = sigRS[int(startS):self.NF+int(startS)]
		    xT = sigTS[int(startS):self.NF+int(startS)]
		...
		print('Audio Processed! (Kabal, section 2), ' + str(self.Np) + ' windows processed ' + \
		  ' in ' + str(time.perf_counter()-startTime) + ' seconds.')
